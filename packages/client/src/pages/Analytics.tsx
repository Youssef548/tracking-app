...
import type { DayCompletion, WeeklyAnalytics, MonthlyAnalytics } from '../hooks/useAnalytics';
...
import type { ComparisonItem } from '../components/ChartBlock';
...
function derivePeriodStats(days: DayCompletion[] | undefined, habits: Habit[]): PeriodStats {
...
function PeriodHeatmap({ days }: PeriodHeatmapProps) {
...
interface HabitBreakdownProps {
  habits: Habit[];
  consistency: WeeklyConsistency | undefined;
}

function HabitBreakdown({ habits, consistency }: HabitBreakdownProps) {
...
export default function Analytics() {
  ...
  const periodData: MonthlyAnalytics | undefined = view === 'month' ? monthly : view === '30days' ? last30 : undefined;
  const periodStats = derivePeriodStats(periodData?.days, habits);

  const weekBarData: ComparisonItem[] = [
    { label: 'THIS WEEK', completed: weekly?.completedCount ?? 0, target: weekly?.targetCount ?? 0 },
  ];
  const periodBarData: ComparisonItem[] = [
