...
import type { Habit, Category, CreateHabitInput } from '@mindful-flow/shared/types';
  ...
  return (
    <>
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold font-headline tracking-tight text-on-surface mb-3">Your Habits</h1>
          <p className="text-on-surface-variant text-lg">Manage, organize, and track what you do every day.</p>
        </div>
          ...
          {categories.map((cat) => (
            <div key={cat._id} className="flex items-center gap-2 bg-surface-container-lowest px-4 py-2.5 rounded-xl border border-outline-variant/10 group">
              ...
              <span className="text-xs text-on-surface-variant">
                {habits.filter((h) => {
                  const hCatId = h.categoryId && '_id' in h.categoryId ? h.categoryId._id : h.categoryId;
                  return hCatId === cat._id;
                }).length} habits
              </span>
              <div className="flex gap-1 ml-1 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
      ...
      <AnimatedList className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
        {habits.map((habit) => {
          ...
          return (
            ...
            <div className="group bg-surface-container-lowest p-7 rounded-2xl border border-outline-variant/10 hover:border-primary/20 transition-all duration-300">
      ...
      {habits.length === 0 && (
        <div className="flex flex-col items-center py-20 text-center">
          ...
          <p className="text-on-surface-variant mb-6 max-w-xs">Start with one daily habit. Small consistency compounds into real results.</p>
          <button onClick={() => { setEditing(null); setModalOpen(true); }}
