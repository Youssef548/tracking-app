...
export default function HabitModal({ open, onClose, onSave, habit = null, isPending = false }: HabitModalProps) {
  ...
  const [trackingType, setTrackingType] = useState<'checkmark' | 'duration'>('checkmark');
  ...
  useEffect(() => {
    if (habit) {
      ...
      setIcon(habit.icon);
      setColor(habit.color as 'primary' | 'secondary' | 'tertiary');
      setDescription(habit.description ?? '');
      setCategoryId(
        (habit.categoryId && '_id' in habit.categoryId ? habit.categoryId._id : habit.categoryId as string | null) ?? ''
      );
      setTrackingType((habit.trackingType ?? 'checkmark') as 'checkmark' | 'duration');
  ...
  const motionProps = shouldReduce
    ? {}
    : { initial: { opacity: 0, scale: 0.97 }, animate: { opacity: 1, scale: 1 }, exit: { opacity: 0, scale: 0.97 }, transition: { duration: 0.2, ease: [0.25, 0.1, 0.25, 1] as [number, number, number, number] } };
