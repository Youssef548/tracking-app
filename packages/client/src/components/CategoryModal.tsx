...
export default function CategoryModal({ open, onClose, onSave, category = null }: CategoryModalProps) {
  const [name, setName] = useState('');
  const [color, setColor] = useState<string>(CATEGORY_COLORS[0] ?? '#8b5cf6');
  ...
  useEffect(() => {
    if (category) {
      ...
      setName('');
      setColor(CATEGORY_COLORS[0] ?? '#8b5cf6');
    }
  }, [category, open]);
