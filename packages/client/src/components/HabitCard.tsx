...
const HabitCard = memo(function HabitCard({ habit, completed, progress = 0, onToggle, weeklyHours = null }: HabitCardProps) {
      ...
      <div className="ml-8">
        {habit.trackingType === 'duration' ? (
          <button onClick={onToggle}
            aria-label={`${habit.name}: ${weeklyHours ?? 0} of ${habit.weeklyTarget ?? 0} hours logged. ${completed ? 'Mark incomplete' : 'Log time'}`}
            ...
            {weeklyHours != null ? `${weeklyHours}/${habit.weeklyTarget ?? 0}h` : `0/${habit.weeklyTarget ?? 0}h`}
          </button>
        ) : (
          <button onClick={onToggle}
